<template>
  <div class="content">
    <div class="header">
      ...
      <input type="text" class="search" placeholder="Search keywords" v-model="searchterm" @keyup.enter="submit">
    </div>
    <div class="gallery" >
...
export default {
  ...
  methods:{
    getdata(search){
       this.$axios.get(`https://api.unsplash.com/search/photos?page=1&per_page=30&query=${search}&order_by=popular`,{
            headers: {
              Authorization:
    ...
    addlike(picture){
      ...
      else {this.$store.commit('saved/remove', picture.id)}
    },
    issaved(id){
